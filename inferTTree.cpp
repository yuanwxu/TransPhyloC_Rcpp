/*
   ...
   - Finished outbreak only, use dateT = Inf

   - Omitting startCTree and allowTransPostSamp

   - Must specify dateT = Inf since not all c++ default arguments can be parsed into R equivalents. It is repositioned
   in the secound argument to inferTTreeC.

   - Must specify shSam and scSam, shape and scale parameter of sampling time density since C++ language rules on default
   arguments.

   - Ignoring "source" component of MCMC output, since need understanding of original R code 
